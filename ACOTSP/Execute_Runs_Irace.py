...
for base_dir, dest_dir, scenario in directories:
    ...
    for seed in seeds:
        ...
        path = os.path.join(dest_dir, f"{scenario}_seed_{seed}")
...
with concurrent.futures.ThreadPoolExecutor(max_workers=K) as executor:
    futures = {executor.submit(execute_scenario, path, seed, run_id, train_dir, parallel): [path, seed, run_id] for path, seed, run_id in runs_data}
